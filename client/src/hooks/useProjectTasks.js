...
//             console.log(`useProjectTasks: Fetching tasks for project ID: ${project._id}`);
//             const res = await axios.get(`${API_URL}/tasks/project/${project._id}`, {
//                 headers: { Authorization: `Bearer ${token}` }
//             });
//             setTasks(res.data);
//             console.log(`useProjectTasks: Tasks fetched successfully for project ${project.name || 'N/A'}:`, res.data.length, "tasks.");
//         } catch (err) {
//             console.error(`useProjectTasks: Failed to fetch tasks for project ${project?.name || 'N/A'}:`, err);
...
//             if (key === '_id' || key === 'projectId' || key === 'userId' || key === '__v') continue;
